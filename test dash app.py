...
app.layout = dbc.Container([
    ...
    # Hidden div for CSV download
    dcc.Download(id="download-dataframe-csv")
], fluid=True)

def get_bye_weeks():
    """Get NFL bye weeks for 2025 season"""
    # 2025 NFL bye weeks (update these based on actual schedule)
    bye_weeks = {
        5: ['Browns', 'Packers', 'Raiders', 'Seahawks'],
        6: ['Chiefs', 'Dolphins', 'Rams', 'Vikings'],
        7: ['Bears', 'Bengals', 'Giants', 'Texans'],
        9: ['Lions', 'Steelers', 'Cardinals', '49ers'],
        10: ['Broncos', 'Patriots', 'Commanders', 'Colts'],
        11: ['Jaguars', 'Ravens', 'Chargers', 'Titans'],
        12: ['Falcons', 'Bills', 'Jets', 'Saints'],
        14: ['Buccaneers', 'Panthers', 'Cowboys', 'Eagles']
    }
    return bye_weeks
...
def create_person_pick_card(person, game_row, side):
    """Create a pick card for a person aligned to away/home side"""
    person_pick_col = f'{person}_pick'
    person_pick = game_row[person_pick_col] if pd.notna(game_row[person_pick_col]) else None
    
    # Determine if this person picked the away or home team
    ...
    if person_pick:
        ...
        if (person_pick == away_team or 
            (away_team and away_team.lower() in person_pick.lower()) or
            (person_pick.lower() in away_team.lower())):
            picked_away = True
        elif (person_pick == home_team or 
              (home_team and home_team.lower() in person_pick.lower()) or
              (person_pick.lower() in home_team.lower())):
            picked_home = True
    
    # Only show card if person picked the team for this side
    if (side == 'away' and not picked_away) or (side == 'home' and not picked_home):
        return html.Div(style={'height': '35px', 'margin': '2px'})  # Empty spacer
    
    if not person_pick:
        return html.Div("No pick", style={
    ...
    return html.Div([
        html.Div([
            html.Div(person.title(), style={'fontSize': '10px', 'fontWeight': 'bold', 'marginBottom': '2px'}),
            html.Div([
                html.Img(src=pick_logo_url, style={'height': '16px', 'marginRight': '4px'}) if pick_logo_url else "",
                html.Span(person_pick, style={'fontSize': '11px'})
...
# Update callback with auto-refresh
@app.callback(
...
def update_status(n_clicks, n_intervals):
    ...
    # Only proceed if button clicked or auto-refresh interval
    if trigger_id == 'update-btn' and not n_clicks:
        return ""
    ...
    try:
        ...
        if trigger_id == 'interval-component':
            message = f"Auto-updated at {timestamp}: {message}"
...
def render_leaderboard_tab():
    ...
    return [
        ...
        dbc.Row([
            dbc.Col([
                dbc.Button("📥 Export All Data to CSV", id="btn-csv", color="primary", className="mb-3")
...
def update_weekly_picks_content(selected_week):
    ...
    try:
        ...
        bye_teams = bye_weeks.get(selected_week, [])
        ...
        # Create bye week display
        bye_week_display = []
        if bye_teams:
            bye_week_display = dbc.Alert([
                html.Strong("Teams on Bye: "),
                ", ".join(bye_teams)
        ...
        for i, row_data in enumerate(picks_data):
            ...
            # Winner display with scores
            if pd.notna(game_row['actual_winner']) and game_row['actual_winner'] != 'TBD':
                score_text = ""
                if pd.notna(game_row.get('away_score')) and pd.notna(game_row.get('home_score')):
                    score_text = f" ({game_row['away_score']}-{game_row['home_score']})"
                
                winner_display = html.Div([
                    html.Img(src=winner_logo_url, style={'height': '25px', 'marginRight': '8px'}) if winner_logo_url else "",
                    html.Span(f"Winner: {game_row['actual_winner']}{score_text}", style={'fontWeight': 'bold', 'color': '#28a745'})
        ...
        return [
            bye_week_display if bye_teams else [],
            dbc.Card([
...
def render_weekly_records_tab():
    try:
        weekly_df, weekly_winners = get_weekly_records_data()
        ...
        # Create style conditions for DataTable
        style_data_conditional = [
            {
                'if': {'row_index': 'odd'},
                'backgroundColor': '#f8f9fa'
        ...
        # Add highlighting for weekly winners
        for week, winners in weekly_winners.items():
            for winner in winners:
                # Highlight the winner's cell for that week
                style_data_conditional.append({
                    'if': {
                        'filter_query': f'{{Week}} = "Week {week}"',
                        'column_id': winner
                    },
                    'backgroundColor': '#ffd700',  # Gold color for winners
                    'color': 'black',
                    'fontWeight': 'bold',
                    ...
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            dbc.Alert([
                                ...
                                html.Span("Gold", style={'backgroundColor': '#ffd700', 'padding': '2px 6px', 'fontWeight': 'bold'}),
                                " = Weekly Winner"
...
def get_weekly_records_data():
    """Get weekly records data with totals row and winner info"""
    try:
        conn = get_db_connection()
        if not conn:
            return pd.DataFrame(), {}
        ...
        weekly_records = []
        weekly_winners = {}  # Track winner for each week
        totals = {'Week': 'TOTALS'}

        for week in weeks:
            week_df = df[df['week'] == week]
            week_record = {'Week': f"Week {week}"}
            week_wins = {}  # Track wins for this week
...
def export_csv(n_clicks):
    try:
        ...
        # Get all picks data
        df_picks = pd.read_sql_query("SELECT * FROM picks ORDER BY week, game_id", conn)
        
        # Calculate standings
        standings_df = get_current_standings()
        
        # Get weekly records
        weekly_df, _ = get_weekly_records_data()
        ...
        for _, row in df_picks.iterrows():
            export_row = {
                ...
                'Bobby Pick': row['bobby_pick'],
                'Chet Pick': row['chet_pick'],
                'Clyde Pick': row['clyde_pick'],
                'Henry Pick': row['henry_pick'],
                'Nick Pick': row['nick_pick'],
                'Riley Pick': row['riley_pick'],
                'Actual Winner': row['actual_winner'] if pd.notna(row['actual_winner']) else 'TBD',
                'Away Score': row['away_score'] if pd.notna(row.get('away_score')) else '',
                'Home Score': row['home_score'] if pd.notna(row.get('home_score')) else ''
            }
            export_data.append(export_row)
        
        df_export = pd.DataFrame(export_data)
        
        return dcc.send_data_frame(df_export.to_csv, "nfl_picks_export.csv", index=False)
...
if __name__ == '__main__':
    ...
    app.run_server(debug=False, host='0.0.0.0', port=port)_winner = row['away_team']
                        elif row['actual_winner'] == 'home':
                            actual_team_winner = row['home_team']
                        else:
                            actual_team_winner = row['actual_winner']
                    ...
                    week_wins[person.title()] = 0

            # Determine weekly winner(s)
            if week_wins:
                max_wins = max(week_wins.values())
                weekly_winners[week] = [name for name, wins in week_wins.items() if wins == max_wins]
        ...
        return pd.DataFrame(weekly_records), weekly_winners
