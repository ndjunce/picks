...
def compare_lists(set1_overall, set2_overall, set1_no_qb, set2_no_qb, set1_pos, set2_pos):
    ...
    comparison += "Set 1 (with Cheatsheet Dynasty Superflex):\n"
    comparison += str(set1_overall[['Name', 'Pos', 'Projected_Points', 'Tier', 'Rank']].head(10)) + "\n\n"
    comparison += "Set 2 (without Cheatsheet):\n"
    ...
    comparison += f"Players in Set 1 top 10 not in Set 2: {diff1}\n"
    comparison += f"Players in Set 2 top 10 not in Set 1: {diff2}\n"
    return comparison
...
with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    ...
    df_overall1_set, df_no_qb1, pos_rank1 = generate_rankings('set1', df_overall1, positional1, df_dst1, is_superflex=True)
    df_overall1_set.to_excel(writer, sheet_name='Set1_Overall', index=False)
    df_no_qb1.to_excel(writer, sheet_name='Set1_No_QB', index=False)
    for pos, df in pos_rank1.items():
        df.to_excel(writer, sheet_name=f'Set1_{pos}', index=False)
    df_overall2 = df_overall_r
    positional2 = positional_r
    df_dst2 = df_dst1
    df_overall2_set, df_no_qb2, pos_rank2 = generate_rankings('set2', df_overall2, positional2, df_dst2)
    df_overall2_set.to_excel(writer, sheet_name='Set2_Overall', index=False)
    df_no_qb2.to_excel(writer, sheet_name='Set2_No_QB', index=False)
    for pos, df in pos_rank2.items():
        df.to_excel(writer, sheet_name=f'Set2_{pos}', index=False)
