...
# Weekly Picks Tab - FIXED VERSION
...
def update_weekly_picks_content(selected_week):
    ...
    try:
        ...
        for _, row in df.iterrows():
            away_logo_url = get_team_logo_url(row['away_team'])
            home_logo_url = get_team_logo_url(row['home_team'])
            winner_logo_url = get_team_logo_url(row['actual_winner']) if pd.notna(row['actual_winner']) else None
            
            # Create game header with scores and logos
            if (pd.notna(row.get('away_score')) and pd.notna(row.get('home_score')) and 
                row.get('away_score') is not None and row.get('home_score') is not None):
                # Game completed with scores
                game_header = html.Div([
                    html.Div([
                        html.Img(src=away_logo_url, style={'height': '30px', 'marginRight': '8px'}) if away_logo_url else "",
                        html.Strong(f"{row['away_team']} {int(row['away_score'])}")
                    ], style={'display': 'flex', 'alignItems': 'center', 'flex': '1'}),
                    
                    html.Div("@", style={'margin': '0 15px', 'fontSize': '18px', 'fontWeight': 'bold'}),
                    
                    html.Div([
                        html.Strong(f"{int(row['home_score'])} {row['home_team']}"),
                        html.Img(src=home_logo_url, style={'height': '30px', 'marginLeft': '8px'}) if home_logo_url else ""
                    ], style={'display': 'flex', 'alignItems': 'center', 'flex': '1', 'justifyContent': 'flex-end'})
                ], style={'display': 'flex', 'alignItems': 'center', 'marginBottom': '15px', 'padding': '10px', 'backgroundColor': '#e8f5e8', 'borderRadius': '5px', 'border': '2px solid #28a745'})
            else:
                # Game not yet completed
                game_header = html.Div([
                    html.Div([
                        html.Img(src=away_logo_url, style={'height': '30px', 'marginRight': '8px'}) if away_logo_url else "",
                        html.Strong(row['away_team'])
                    ], style={'display': 'flex', 'alignItems': 'center', 'flex': '1'}),
                    
                    html.Div("@", style={'margin': '0 15px', 'fontSize': '18px', 'fontWeight': 'bold'}),
                    
                    html.Div([
                        html.Strong(row['home_team']),
                        html.Img(src=home_logo_url, style={'height': '30px', 'marginLeft': '8px'}) if home_logo_url else ""
                    ], style={'display': 'flex', 'alignItems': 'center', 'flex': '1', 'justifyContent': 'flex-end'})
            ...
            if pd.notna(row['actual_winner']) and row['actual_winner'] != 'TBD':
                if (pd.notna(row.get('away_score')) and pd.notna(row.get('home_score')) and 
                    row.get('away_score') is not None and row.get('home_score') is not None):
                    total_score = int(row['away_score']) + int(row['home_score'])
                    winner_text = f"Final: {row['actual_winner']} wins | Total: {total_score} points"
                else:
                    winner_text = f"Winner: {row['actual_winner']}"
                
                winner_display = html.Div([
                    html.Img(src=winner_logo_url, style={'height': '25px', 'marginRight': '8px'}) if winner_logo_url else "",
                    html.Span(winner_text, style={'fontWeight': 'bold', 'color': '#28a745'})
                ], style={'display': 'flex', 'alignItems': 'center', 'marginBottom': '10px'})
            else:
                winner_display = html.Div("Game not completed", style={'fontStyle': 'italic', 'color': '#6c757d', 'marginBottom': '10px'})
            
            # Create aligned picks display
            picks_display = html.Div([
                # Away team picks column
                html.Div([
                    html.Div("Away Team Picks", style={'fontWeight': 'bold', 'marginBottom': '8px', 'textAlign': 'center', 'color': '#495057'}),
                    *[create_person_pick_card(person, row, 'away') for person in people]
                ], style={'flex': '1', 'padding': '0 10px'}),
                
                # VS divider
                html.Div([
                    html.Div("VS", style={'fontWeight': 'bold', 'fontSize': '18px', 'color': '#6c757d'})
                ], style={'display': 'flex', 'alignItems': 'center', 'justifyContent': 'center', 'padding': '0 15px'}),
                
                # Home team picks column  
                html.Div([
                    html.Div("Home Team Picks", style={'fontWeight': 'bold', 'marginBottom': '8px', 'textAlign': 'center', 'color': '#495057'}),
                    *[create_person_pick_card(person, row, 'home') for person in people]
                ], style={'flex': '1', 'padding': '0 10px'})
...
def create_person_pick_card(person, game_row, side):
    ...
    # Compare the person's pick with the actual team names
